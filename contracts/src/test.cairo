use snforge_std::{
    BlockId, declare, ContractClassTrait, DeclareResultTrait, start_cheat_caller_address,
    stop_cheat_caller_address, store, map_entry_address
};
use starknet::ContractAddress;
use openzeppelin::token::erc20::interface::{IERC20Dispatcher, IERC20DispatcherTrait};
use contracts::Borrow::{IBorrowerDispatcher, IBorrowerDispatcherTrait};

fn deploy_borrower(
    zklend_market_address: ContractAddress,
    usdc_address: Array<ContractAddress>,
    usdt_address: Array<ContractAddress>
...
fn test_starknet_protocols_2() {
    ...
    let usdc_address: ContractAddress =
        ...
        .try_into()
        .unwrap();
    let usdt_address: ContractAddress =
    ...
    let (borrower_address, borrower_dispatcher) = deploy_borrower(
        zklend_market_address, array![usdc_address], array![usdt_address]
    );
    ...
    // TODO: Check that the market contract obtained the USDC (store the balance before and after
    // the deposit and check the difference)
    ...
    // TODO: Check that the market received the USDT (store the balance before and after the
    // repayment and check the difference)
    assert(
        balance_after_repay - balance_before_repay == fifty_usdt, 'Wrong USDT balance in Market'
    );
